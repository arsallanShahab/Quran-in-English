  ...
  return (
    <>
      <div
        ...
        style={{
          backgroundColor: theme.theme == "default" ? default50 : primary50,
        }}
      >
        <span
          className="cursor-default duration-300"
          style={{ color: theme.theme == "default" ? default900 : primary900 }}
        >
          Quran
        </span>
        <span
          className=" font-normal text-xs lg:text-xl mt-[-0.25em] ml-[6.5em] md:ml-[12em] lg:mt-[-.70em] lg:ml-[7.5em] text-center"
          style={{ color: default400 }}
        >
          with English translation
        </span>
        <span
          className="font-poppins -indent-[8px] text-4xl md:text-4xl font-semibold tracking-[-0.25em] mt-[.3em] px-[30px] py-[15px] rounded-full hover:bg-white focus:scale-75 active:scale-75 duration-300 cursor-pointer"
          style={{ color: theme.theme == "default" ? default900 : primary900 }}
      ...
      <div
        ...
        className={`fixed w-full h-full top-0 bg-gray-900 duration-200 ${
          active ? "backdrop-active" : "backdrop-hidden"
        }`}
      ></div>
      <div className="absolute top-12 right-5 lg:top-10 lg:right-10">
        <div
          className="px-3 py-3 mt-1 text-xs lg:text-sm font-normal bg-white rounded-xl active:scale-75 duration-200 cursor-pointer"
          onClick={() => setActive((prev) => !prev)}
        >
          <svg
            width={25}
            height={25}
            fill="none"
            stroke={`${theme.theme == "default" ? "#000" : primary900}`}
            strokeWidth="2"
            strokeLinecap="round"
      ...
      <div
        className={`sidebar fixed top-0 right-0 h-full duration-300 ${
          active ? "sidebar-active" : "sidebar-hidden"
        }`}
      >
        <div className="w-[300px] rounded-l-lg px-5 h-full bg-gray-100 top-0 right-0">
          ...
          <div className="px-5 bg-white border py-6 rounded-xl">
            ...
            <div className="flex flex-row gap-3 font-semibold text-sm">
              <div
                className="basis-1/2 cursor-pointer inline-block border-2 rounded-lg text-center text-white px-3 py-3 duration-200"
                style={{
                  color: theme.theme != "default" ? "#000" : "#FFF",
                  backgroundColor:
                    theme.theme != "default" ? "#FFF" : default900,
                  transform:
                    theme.theme == "default" ? "scale(0.95)" : "scale(1)",
                }}
                onClick={handleTheme}
                data-handler="default"
              >
                {theme.theme == "default" ? "active" : "Default"}
              </div>
              <div
                className="basis-1/2 cursor-pointer inline-block border-2 rounded-lg text-center text-white px-3 py-3 duration-200"
                style={{
                  color: theme.theme != "default" ? "#FFF" : primary800,
                  backgroundColor: theme.theme != "monet" ? "#FFF" : primary900,
                  transform:
                    theme.theme == "monet" ? "scale(0.95)" : "scale(1)",
                }}
                onClick={handleTheme}
                data-handler="monet"
              >
                {theme.theme == "monet" ? "active" : "Random"}
              </div>
            </div>
          </div>
        </div>
